...
# Species as rownames:
sp_tr_MAMMALS <- tibble::column_to_rownames(sp_tr_MAMMALS,
                                             "Species")

# 3 - Impute traits based on Random Forest approach =====================

# Note: remove fuzzy traits:
fuzzy_tr_df <- sp_tr_MAMMALS %>%
...
sp_tr_MAMMALS_nonfuzzy <- sp_tr_MAMMALS %>%
  dplyr::select(- c("terrestrial",
                    "saxicolous",
                    "arboreal",
                    "cryptic",
...
mice::md.pattern(sp_tr_MAMMALS_nonfuzzy)
## Compute missing data - get 100 distributions:
init_test <- mice::mice(sp_tr_MAMMALS_nonfuzzy, ntree = 300,
                        m = 100,
                        meth = 'rf', seed = 42)
...
# Complete the sp_tr_MAMMALS with imputed traits:
sp_tr_MAMMALS <- tibble::rownames_to_column(sp_tr_MAMMALS,
                                             "Species")
complete_sp_tr_MAMMALS <- sp_tr_MAMMALS %>%
...
complete_sp_tr_MAMMALS <- tibble::column_to_rownames(complete_sp_tr_MAMMALS,
                                                      "Species")
...
## Density plots:
# OffspringPerRepro:
ggplot2::ggplot(data = sp_tr_MAMMALS) +
  ggplot2::geom_density(ggplot2::aes(x = OffspringPerRepro),
                        fill="#69b3a2", alpha = 0.5) +
  ggplot2::geom_density(data = complete_sp_tr_MAMMALS,
...
plot_sp_tr_MAMMALS <- sp_tr_MAMMALS %>%
...
plot_sp_tr_MAMMALS[, which(colnames(plot_sp_tr_MAMMALS) != c("Species"))] <- apply(plot_sp_tr_MAMMALS[, which(colnames(plot_sp_tr_MAMMALS) != c("Species"))],
                                                                                     2,
                                                                                     as.numeric)
# Format to plot:
plot_sp_tr_MAMMALS <- plot_sp_tr_MAMMALS %>%
...
# SVLMax
ggplot2::ggplot(data = plot_sp_tr_MAMMALS[which(plot_sp_tr_MAMMALS$traits_nm == "SVLMax"), ]) +
...
# ReproPerYear
ggplot2::ggplot(data = plot_sp_tr_MAMMALS[which(plot_sp_tr_MAMMALS$traits_nm == "ReproPerYear"), ]) +
  ggplot2::geom_jitter(ggplot2::aes(x = traits_nm,
                                    y = values),
                       color = "#69b3a2",
                       alpha = 0.6) +
...
# LongevityMax
ggplot2::ggplot(data = plot_sp_tr_MAMMALS[which(plot_sp_tr_MAMMALS$traits_nm == "LongevityMax"), ]) +
  ggplot2::geom_jitter(ggplot2::aes(x = traits_nm,
                                    y = values),
                       color = "#69b3a2",
                       alpha = 0.6) +
  ggplot2::geom_jitter(data = plot_imputed_traits[which(plot_imputed_traits$traits_nm == "LongevityMax"), ],
                       ...
                       alpha = 0.6)
# ActiviySeasonLength:
ggplot2::ggplot(data = plot_sp_tr_MAMMALS[which(plot_sp_tr_MAMMALS$traits_nm == "ActivitySeasonLength"), ]) +
  ggplot2::geom_jitter(ggplot2::aes(x = traits_nm,
                                    y = values),
                       color = "#69b3a2",
                       alpha = 0.6) +
...
# OffspringPerRepro:
ggplot2::ggplot(data = plot_sp_tr_MAMMALS[which(plot_sp_tr_MAMMALS$traits_nm == "OffspringPerRepro"), ]) +
  ggplot2::geom_jitter(ggplot2::aes(x = traits_nm,
                                    y = values),
                       color = "#69b3a2",
                       alpha = 0.6) +
...
str(complete_sp_tr_MAMMALS)


# Save imputed traits:
saveRDS(complete_sp_tr_MAMMALS, here::here("transformed_data",
